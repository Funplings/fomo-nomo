...
async function refresh(){const version=++refreshVersion;const button=$("refresh"),status=$("status");button.disabled=true;button.textContent="Refreshing...";status.className="status";status.textContent="Fetching public sources";try{const response=await fetch("/api/events",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(state)});const data=await response.json();if(version!==refreshVersion)return;if(!response.ok)throw new Error(data.error||"Could not refresh events.");events=data.events||[];renderCalendar();status.textContent=data.failures?.length?data.failures.length+" source failed to load":"Updated just now"}catch(error){if(version!==refreshVersion)return;status.className="status error";status.textContent=error.message;$("calendar").innerHTML='<div class="message error">'+esc(error.message)+"</div>"}finally{if(version===refreshVersion){button.disabled=false;button.textContent="Refresh"}}}
